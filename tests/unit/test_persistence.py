...
class TestPickle:
    ...
    def test_pickle_from_0_5_keeps_the_row_count_not_the_data(self):
        # tabpfn-client 0.5.0 and 0.5.1 pickled the training matrix itself, as
        # `_last_train_X`. Unpickling keeps what the row-budget check needs and
        # drops the matrix.
        legacy = TabPFNRegressor()
        legacy.model_id_ = MODEL_ID
        vars(legacy)["_last_train_X"] = np.random.randn(7, 2)

        restored = pickle.loads(pickle.dumps(legacy))

        check_is_fitted(restored)
        assert restored._n_train_rows == 7
        assert "_last_train_X" not in vars(restored)
