...
def _installed_version() -> str:
    # Imported here rather than at module level: the package's `__init__`
    # imports this module.
...
def _read_record(source: str | Path | dict[str, Any]) -> _ModelRecord:
    ...
    if isinstance(source, dict):
        ...
        try:
            ...
            # A pickle or joblib file fails to decode before it fails to parse.
            raise ValueError(
...
class ModelPersistenceMixin(BaseEstimator):
    ...
    def __setstate__(self, state: dict[str, Any]) -> None:
        # tabpfn-client 0.5.0 and 0.5.1 pickled the training matrix itself, as
        # `_last_train_X`. Only its row count is fitted state.
        legacy_train_X = state.pop("_last_train_X", None)
        if legacy_train_X is not None:
            state.setdefault("_n_train_rows", legacy_train_X.shape[0])
